...
Only init and doctor are implemented in Phase 1. The rest are wired up as
real subcommands (so `daylog --help` and `daylog <cmd> --help` already show
the full shape of the tool) but print a friendly "not implemented yet"
message and exit 1 instead of doing anything — never an unhandled
traceback.
"""
    ...
    if session_type.lower() == "wayland":
        results.append(
            _check(
                "Display server",
                False,
                "Wayland detected — window titles will be unavailable under Wayland. "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_track.set_defaults(func=_not_implemented("track", "Phase 2"))
    ...
    p_status.set_defaults(func=_not_implemented("status", "Phase 2"))
